...
locals {
  ...
  lambda_name              = "lambda_${var.stream_name}_consumer"
  ...
  kinesis_stream_arn       = data.terraform_remote_state.nikita_infra_state.outputs.transaction_stream_arn
  kinesis_stream_name      = data.terraform_remote_state.nikita_infra_state.outputs.transaction_stream_name
