...
module "vpc_peering" {
  source    = "../../../modules/terraform/aws/vpc_peering"
  providers = {
  ...
  initiator_private_rt_id = "${length(module.internal_private_route.route_table_id) > 0 ? element(concat(module.internal_private_route.route_table_id, list("")), 0) : ""}"
