...
locals {
  // Network
  azs                      = var.azs
  discover-vpc = data.terraform_remote_state.discover_infra_state.outputs.vpc_id
  public_subnet_ids        = data.terraform_remote_state.discover_infra_state.outputs.public_subnet_ids
  private_subnet_ids       = data.terraform_remote_state.discover_infra_state.outputs.private_subnet_ids
}

locals {
  // Load balancer, Autoscaling group
  lb_name                  = "${var.service_name}-${terraform.workspace}"
  security_groups_asg      = data.terraform_remote_state.discover_infra_state.outputs.autoscalinggroup_sg_id
  security_groups_alb      = data.terraform_remote_state.discover_infra_state.outputs.alb_sg_id
  alb_targetgroups_arns    = data.terraform_remote_state.discover_infra_state.outputs.alb_targetgroups_arns
  alb_targetgroup_names    = data.terraform_remote_state.discover_infra_state.outputs.alb_targetgroup_names
  asg_name                            = data.terraform_remote_state.discover_infra_state.outputs.asg_name
  ...
  loadbalancer                        = data.terraform_remote_state.discover_infra_state.outputs.alb_dnsname
...
locals {
  // ECS
  cluster_name             = data.terraform_remote_state.discover_infra_state.outputs.ecs_cluster_name
  ...
  iam_instance_profile     = data.terraform_remote_state.discover_infra_state.outputs.ecs_instance_profile_name
  ecs_task_role            = data.terraform_remote_state.discover_infra_state.outputs.ecs_task_role_arn
  ecs_task_execution_role  = data.terraform_remote_state.discover_infra_state.outputs.ecs_task_execution_role_arn
  ecs_service_role         = data.terraform_remote_state.discover_infra_state.outputs.ecs_service_role_name
  ecs_service_role_arn     = data.terraform_remote_state.discover_infra_state.outputs.ecs_service_role_arn
  ...
  private_registry_token   = data.terraform_remote_state.cloud_sre_aws_state.outputs.private_registry_arn
  ecs_service_name = module.discover-ecs-service.ecs_service
}
