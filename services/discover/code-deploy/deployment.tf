data "terraform_remote_state" "discover_infra_state" {
  backend = "remote"
  config = {
    organization = "discover"
    token        = var.TF_TOKEN_DISCOVER
    workspaces = {
      name = "discover-infra-${terraform.workspace}"
    }
  }
}
...
module "ecs_service_autoscaling_policy_scale_up" {
  name                         = "${module.discover-ecs-service.ecs_service}-scale-up"
...
module "ecs_service_autoscaling_policy_scale_down" {
  name                           = "${module.discover-ecs-service.ecs_service}-scale-down"
...
module "cloudwatch_alarm_ecs_service_scaledown" {
  ...
  alarm_description   = "This alarm monitors ${local.ecs_service_name} containers CPU utilization for scaling down"
...
output "ecs_container_definition" {
  value = module.discover-ecs-service.ecs_container_definition
}
output "ecs_task_definition_arn" {
  value = module.discover-ecs-service.ecs_task_definition_arn
}

output "ecs_Service_name" {
  value = module.discover-ecs-service.ecs_service
}
